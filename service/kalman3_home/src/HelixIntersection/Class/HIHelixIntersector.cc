   ...
   const KTMoveControl::MoveDirection aDirection )
{
   if ( m_printDiagnostics )
      cout << "entering swimToNextIntersection" << endl;
...
//       }
	 ...
	 else
	 {
	    ...
	    if( surfaceIncrement == 0 )
	    {
		  ...
		  delete savedHelix ;
		  if ( m_printDiagnostics )
		     cout << "swimToNextIntersection exit point A" << endl;
		  return kNoMoreSurfaces ;
	       }
	    }
	    else
      ...
      if ( m_printDiagnostics )
	 cout << "just before m_helix->position()" << endl;
      Hep3Vector oldPos = m_helix->position() ;
      if ( m_printDiagnostics )
	 cout << "just after m_helix->position()" << endl;
...
// 		 << " " << ( *m_currentSurface )->outerMaterial()->name()
	 ...
	 if( tryCurling && oldMomDotPos * newMomDotPos > 0. )
	 {
	    ...
	    {
	       if ( m_printDiagnostics )
		  cout << "swimToNextIntersection exit point B" << endl;
	       return kNoMoreSurfaces ;
	    }
	    else
	    {
	       if ( m_printDiagnostics )
		  cout << "swimToNextIntersection exit point C" << endl;
		    ...
		    || lastArc > 1000. || lastArc < 1.e-10 ) )
	 {
	    ...
	    if( tryCurling )
	    {
	       delete savedHelix ;
	       if ( m_printDiagnostics )
		  cout << "swimToNextIntersection exit point D" << endl;
	       return kNoMoreSurfaces ;
//	       return kHelixTransportError ;
	    }
	 ...
	 {
	    if ( m_printDiagnostics )
	       cout << "swimToNextIntersection exit point E" << endl;
	    return kNoMoreSurfaces ;
	 }
	 else
	 {
	    if ( m_printDiagnostics )
	       cout << "swimToNextIntersection exit point F" << endl;
	    return kHelixTransportError ;
	 }
      }
...
//	 m_helix->revertToSnapshot( savedHelix ) ;
	 ...
	 delete savedHelix ;
	 if ( m_printDiagnostics )
	    cout << "swimToNextIntersection exit point G" << endl;
   ...
   delete savedHelix ;
   if ( m_printDiagnostics )
      cout << "swimToNextIntersection exit point H" << endl;
   return status ;
}
