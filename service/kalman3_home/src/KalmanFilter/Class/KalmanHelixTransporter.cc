   ...
   HepPoint3D& aDestinationRefPt ) const
{
   if ( m_printDiagnostics )
      cout << "Entering transportThroughMaterial" << endl;

   if( !m_surfacesOK )
   {
      if ( m_printDiagnostics )
	 cout << "transportThroughMaterial exit point A" << endl;
      return false ;
   }
   ...
   if( helix->momentum().mag2() == 0. )
   {
      delete helix ;
      if ( m_printDiagnostics )
	 cout << "transportThroughMaterial exit point B" << endl;
      return false ;
   }
       ...
       fabs( helix->lastArcLength() ) > oneRevolution )
   {
      delete helix ;
      if ( m_printDiagnostics )
	 cout << "transportThroughMaterial exit point C" << endl;
      return 0 ;
   }
   ...
   if( momDotPos * momDotPosInit > 0. )
   {
	 ...
	 if ( m_printDiagnostics )
	    cout << "Calling swimToNextIntersection" << endl;
	 status = intersector.swimToNextIntersection( moveDir ) ;
	 if ( m_printDiagnostics )
	    cout << "Finished swimToNextIntersection" << endl;
   ...
   if( transportFailed || lastMaterial.isNull() )
   {
      delete helix ;
      if ( m_printDiagnostics )
	 cout << "transportThroughMaterial exit point D" << endl;
      return false ;
   }
   ...
   *helix = lastHelix ;
       ...
       KTMoveControl::kMoveOK )
   {
      delete helix ;
      if ( m_printDiagnostics )
	 cout << "transportThroughMaterial exit point E" << endl;
      return false ;
   }
       ...
       || !helix->errorMatrixOK() )
   {
      delete helix ;
      if ( m_printDiagnostics )
	 cout << "transportThroughMaterial exit point F" << endl;
      return false ;
   }
   ...
   delete helix ;
   if ( m_printDiagnostics )
      cout << "transportThroughMaterial exit point G" << endl;
   return true ;
}
