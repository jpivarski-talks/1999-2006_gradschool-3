   ...
   m_sagitta( aSagitta )
{
   ...
   // this is going to be an intersector for a cylinder rather than a
   // plane, to model the sagitta observed in the sensors (if it's
   // nonzero)
   if ( m_sagitta != 0. ) {
      const HepPoint3D sensor_center = aSensor.coorInSi();
      const Hep3Vector sensor_normal = aSensor.ladderOwner().unitVectorYInSi();
      const double sensor_halfwidth = aSensor.halfLengthX();
      
      m_cylinder_radius = ( sqr( sensor_halfwidth ) + sqr( m_sagitta ) ) / ( 2 * m_sagitta );

      m_cylinder_center = sensor_center - m_cylinder_radius * sensor_normal;

      m_cylinder_dircos = aSensor.ladderOwner().unitVectorZInSi();
   }
   // if m_sagitta is zero, we revert to planar behaviour (in propogateHelix)
...
DABoolean
HISiSensorPlane::propagateHelix( HIHelix &helixToPropagate,
				 KTMoveControl::MoveDirection aDirection )
{
   if ( m_sagitta == 0. )
      return HISiLadderPlane::propagateHelix( helixToPropagate, aDirection );

   KTMoveControl::MoveStatus status(
      helixToPropagate.moveToCylinder( m_cylinder_center, m_cylinder_dircos, m_cylinder_radius, aDirection ) );

   return ( status == KTMoveControl::kMoveOK );
}
...
void
HISiSensorPlane::setSagitta( const Meters aSagitta )
{
   m_sagitta = aSagitta;
}
...
//      ( m_transToSi * m_currentHit->calibratedSVHit()->localcoord() ) ;
   ...
   Hep3Vector residualVector = aHelix.position() -
      ( m_transToSi * m_currentHit->calibratedSVHit()->localcoord() + m_CorrectionInSi ) ;

   return m_currentHit->measurementDirection().dot( residualVector ) ;
   // the first curvature correction: dca -> ( dca + dca^3/24/radius^2 )
   // only becomes a 0.1% effect for residuals 1 cm and above (where
   // sagitta is 1000 microns).
}
