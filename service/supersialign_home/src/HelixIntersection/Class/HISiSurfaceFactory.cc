...

{
   ...
   // copy the sagitta table
   for( int j = 0;  j < SiGeomDescription::kNLadders;  j++ )
      if ( aSagittaTable == NULL )
	 m_sagitta[j] = 0.;
      else
	 m_sagitta[j] = aSagittaTable[j];
}
...
// Make the rphi hit surface and hit surface boundaries.     
	...
	++rphiLadderItr, ++matchedHitsItr )
   {
	 ...
	 new HISiSensorPlane( m_sensorMaterial,
			      ...
			      tempZHit, 
			      correctionInSi,
			      m_sagitta[ (* rphiLadderItr)->number() - 1 ] ) ;  
      rphiHitCount++;
...
// Now do similar for Z...
       ...
       ++zLadderItr, zItr++ )
   {
	 ...
	 new HISiSensorPlane( m_sensorMaterial,
			      ...
			      *zItr,
			      HepPoint3D(0.,0.,0.),
			      m_sagitta[ (* zLadderItr)->number() - 1 ] ); //No Rphi hits on this surface!
