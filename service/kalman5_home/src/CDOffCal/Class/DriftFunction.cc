   ...
   m_CDOffCal( CDOffCal )
{
  ...
  for(FAConstants<DBDRWeightLayer>::const_iterator 
	wloopy1  = m_dataHolder->LayerWeight().begin() ;
        wloopy1 != m_dataHolder->LayerWeight().end()   ; 
      ++wloopy1 ) {
     report(INFO, kFacilityString) << "DRWEIGHTLAYER " << wloopy1->get_layer() << " "
	  << wloopy1->get_fittingweight() << endl;
  }

  for(FAConstants<DBDRWeightDrift>::const_iterator 
	wloopy2 =m_dataHolder->DriftWeight().begin();
        wloopy2!=m_dataHolder->DriftWeight().end()  ; 
      ++wloopy2 ) {
       report(INFO, kFacilityString) << "DRWEIGHTDRIFT " << wloopy2->get_layer() << " "
	    << wloopy2->get_bin() << " "
	    << wloopy2->get_fittingweight() << endl;
  }

  for(FAConstants<DBDRWeightTheta>::const_iterator 
	wloopy3 =m_dataHolder->ThetaWeight().begin();
        wloopy3!=m_dataHolder->ThetaWeight().end()  ; 
      ++wloopy3 ) {
       report(INFO, kFacilityString) << "DRWEIGHTTHETA " << wloopy3->get_layer() << " "
	    << wloopy3->get_bin() << " "
	    << wloopy3->get_fittingweight() << endl;
  }

  for(FAConstants<DBDRWeightPhi>::const_iterator 
        wloopy4 =m_dataHolder->PhiWeight().begin();
        wloopy4!=m_dataHolder->PhiWeight().end()  ; 
      ++wloopy4 ) {
       report(INFO, kFacilityString) << "DRWEIGHTPHI " << wloopy4->get_layer() << " "
	    << wloopy4->get_bin() << " "
	    << wloopy4->get_fittingweight() << endl;
  }
