...
//	return ((float)numberHits()) / ((float)m_numberHitsExpected);
	...
	return m_numberIterations;
	 ...
	 return m_detector ;
      ...
      /// Return the number of layers hit *inside* layerNum (EXCLUDING layerNum)
      int layersHitInside( TRSubdetectorLists::HitTypes aHitType,
			   unsigned int layerNum ) const;

      /// Return the number of layers hit *outside* layerNum (EXCLUDING layerNum)
      int layersHitOutside( TRSubdetectorLists::HitTypes aHitType,
			    unsigned int layerNum ) const;
