...
// $Id: HISingleWireCylinder.cc,v 1.17 2003/10/15 19:10:23 mccann Exp $
//
// Revision history
//
// $Log: HISingleWireCylinder.cc,v $
// Revision 1.17  2003/10/15 19:10:23  mccann
// Wrapped excessive DEBUG printouts with m_printDiagnostics==true requirement
//
// Revision 1.16  2003/09/01 19:50:16  wsun
// * HIDRLayerCylinder --> HIWireLayerCylinder
// * HICalibratedDRHit --> HICalibratedWireHit
//
// Revision 1.15  2003/08/30 00:24:18  wsun
...
//
// Revision 1.14  2003/07/25 23:21:23  mccann
...
HISingleWireCylinder::HISingleWireCylinder(
   ...
   : HIWireLayerCylinder( aInnerMaterial,
			  aOuterMaterial,
			  aWireLayer,
			  aLayerNumber,
			  aTransform,
   ...
   m_checkSingleDirectionSwitch( aCheckDirectionSwitch )
{
   if ( printDiagnostics() ) {
	 ...
	 << "aHit == " << aHit << ", aSignedDriftDistance == " << aSignedDriftDistance << endl;
   }
   ...
   if ( printDiagnostics() ) {
      report(DEBUG, kFacilityString)
	 << "constructor: m_pDRHit == " << m_pDRHit;
      if (m_pDRHit != 0) {
	 report(DEBUG, kFacilityString)
	    << ", m_pDRHit->calibratedDRHit() == " << m_pDRHit->calibratedDRHit();
      }
...
void
HISingleWireCylinder::setHICalibratedWireHit( HICalibratedWireHit* aHit )
{
   m_pDRHit = aHit ;
   extractWireInfo() ;
   ...
   if ( printDiagnostics() ) {
      report(DEBUG, kFacilityString)
	 << "setHICalibratedWireHit: m_pDRHit == " << m_pDRHit;
      if (m_pDRHit != 0) {
	 report(DEBUG, kFacilityString)
	    << ", m_pDRHit->calibratedDRHit() == " << m_pDRHit->calibratedDRHit();
      }
...
HISingleWireCylinder::setCalibratedDRHit( const CalibratedDRHit* aHit )
{
   ...
   if ( printDiagnostics() ) {
      report(DEBUG, kFacilityString)
	 << "setCalibratedDRHit: m_pDRHit == " << m_pDRHit;
      if (m_pDRHit != 0) {
	 report(DEBUG, kFacilityString)
	    << ", m_pDRHit->calibratedDRHit() == " << m_pDRHit->calibratedDRHit();
      }
...
HISingleWireCylinder::setSignedDriftDistance( Meters aSignedDriftDistance )
{
   ...
   if ( printDiagnostics() ) {
      report(DEBUG, kFacilityString)
	 << "setSignedDriftDistance: m_pDRHit == " << m_pDRHit;
      if (m_pDRHit != 0) {
	 report(DEBUG, kFacilityString)
	    << ", m_pDRHit->calibratedDRHit() == " << m_pDRHit->calibratedDRHit();
      }
...
{
   if ( printDiagnostics() ) {
      report(DEBUG, kFacilityString) << "in extractWireInfo" << endl;
   }
   ...
   {
      if ( printDiagnostics() ) {
	 report(DEBUG, kFacilityString) << "m_pDRHit->calibratedDRHit() was non-null, overwriting m_wireInfo (it's okay)" << endl;
      }
   ...
   if ( printDiagnostics() ) {
      report(DEBUG, kFacilityString)
	 << "extractWireInfo: m_pDRHit == " << m_pDRHit;
      if (m_pDRHit != 0) {
	 report(DEBUG, kFacilityString)
	    << ", m_pDRHit->calibratedDRHit() == " << m_pDRHit->calibratedDRHit();
      }
   ...
   KTMoveControl::MoveDirection aDirection )
{
   if ( printDiagnostics() ) {
      report(DEBUG, kFacilityString)
	 << "begin propagateHelix: m_pDRHit == " << m_pDRHit;
      if (m_pDRHit != 0) {
	 report(DEBUG, kFacilityString)
	    << ", m_pDRHit->calibratedDRHit() == " << m_pDRHit->calibratedDRHit();
      }
   ...
   if( m_pDRHit == 0 )
   {
	 ...
	 m_pDRHit = &m_drHit ;
      ...
      {
	 return HIWireLayerCylinder::propagateHelix( helixToPropagate,
						     aDirection ) ;
      }
   }
   ...
   if ( printDiagnostics() ) {
      report(DEBUG, kFacilityString)
	 << "propagateHelix 2: m_pDRHit == " << m_pDRHit;
      if (m_pDRHit != 0) {
	 report(DEBUG, kFacilityString)
	    << ", m_pDRHit->calibratedDRHit() == " << m_pDRHit->calibratedDRHit();
      }
   ...
   if ( printDiagnostics() ) {
      report(DEBUG, kFacilityString)
	 << "propagateHelix 3: m_pDRHit == " << m_pDRHit;
      if (m_pDRHit != 0) {
	 report(DEBUG, kFacilityString)
	    << ", m_pDRHit->calibratedDRHit() == " << m_pDRHit->calibratedDRHit();
      }
   ...
   if ( printDiagnostics() ) {
      report(DEBUG, kFacilityString)
	 << " OINK testHelix.referencePoint() == " << testHelix.referencePoint()
	 << " norm = " << norm << " testHelix.momentumUnit() " << testHelix.momentumUnit()
	 << " momDotNormalOrig == " << momDotNormalOrig
	 << " wire pos == " << wirePosDir.first << endl;
   }
   ...
   if ( printDiagnostics() ) {
      report(DEBUG, kFacilityString)
	 << "BOINK testHelix.referencePoint() == " << testHelix.referencePoint()
	 << " norm = " << norm << " testHelix.momentumUnit() " << testHelix.momentumUnit()
	 << " momDotNormal == " << momDotNormal << endl;

      report(DEBUG, kFacilityString)
	 << "progagateHelix 3.5: m_checkSingleDirectionSwitch == " << m_checkSingleDirectionSwitch
	 << " momDotNormalOrig * momDotNormal == " << momDotNormalOrig << " * " << momDotNormal << endl;
   }
   ...
   if( m_checkSingleDirectionSwitch && momDotNormalOrig * momDotNormal < 0. )
   {
      ...
      if ( printDiagnostics() ) {
	 report(DEBUG, kFacilityString)
	    << "propagateHelix 4: m_pDRHit == " << m_pDRHit;
	 if (m_pDRHit != 0) {
	    report(DEBUG, kFacilityString)
	       << ", m_pDRHit->calibratedDRHit() == " << m_pDRHit->calibratedDRHit();
	 }
   ...
   else
   {
	 ...
	 if( printDiagnostics() )
	 {
      ...
      if ( printDiagnostics() ) {
	 report(DEBUG, kFacilityString)
	    << "propagateHelix 5: m_pDRHit == " << m_pDRHit;
	 if (m_pDRHit != 0) {
	    report(DEBUG, kFacilityString)
	       << ", m_pDRHit->calibratedDRHit() == " << m_pDRHit->calibratedDRHit();
	 }
   ...
   STL_VECTOR( HICalibratedHit* )& aVector )
{
   ...
   if ( printDiagnostics() ) {
      report(DEBUG, kFacilityString)
	 << "addHICalibratedHitsToVector: m_pDRHit == " << m_pDRHit;
      if (m_pDRHit != 0) {
	 report(DEBUG, kFacilityString)
	    << ", m_pDRHit->calibratedDRHit() == " << m_pDRHit->calibratedDRHit();
      }
   ...
   const HIHelix& aHelix, HepVector& outputVector ) const
{
   if ( printDiagnostics() ) {
      report(DEBUG, kFacilityString)
	 << "begin derivatives: m_pDRHit == " << m_pDRHit;
      if (m_pDRHit != 0) {
	 report(DEBUG, kFacilityString)
	    << ", m_pDRHit->calibratedDRHit() == " << m_pDRHit->calibratedDRHit();
      }
