...
// $Id: HICompositePlane.cc,v 1.9 2003/10/14 19:10:52 wsun Exp $
//
// Revision history
//
// $Log: HICompositePlane.cc,v $
// Revision 1.9  2003/10/14 19:10:52  wsun
// Fixed logic bug in determining closest plane to track.
//
...
static const char* const kIdString  = "$Id: HICompositePlane.cc,v 1.9 2003/10/14 19:10:52 wsun Exp $";
				  ...
				  KTMoveControl::MoveDirection aDirection )
{
   ...
   for( ; iter != endSurf ; ++iter )
   {
	 ...
	 if( fabs( tmpArcLength2 ) > 1.e-6 )
	 {
      ...
      {
	 if( fabs( tmpArcLength ) < fabs( bestArcLength ) )
	 {
	 ...
	 else if( fabs( tmpArcLength ) < fabs( nextBestArcLength ) )
	 {
	    nextBestArcLength = tmpArcLength ;
	    nextBestSurface = *iter ;
	 }
      }
   }
