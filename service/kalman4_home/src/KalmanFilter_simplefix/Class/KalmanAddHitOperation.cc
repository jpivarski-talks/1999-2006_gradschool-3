	 ...
	 if( printDiagnostics() )
	 {
	    ...
	    for( int w = 1 ; w <= HIHelix::kNTrackParameters ; ++w )
	    {
		  ...
		  corr0.fast( x, w ) =
		     ...
		     1. / sqrt( fabs( transportedError.fast( x, x ) *
	    ...
	    for( int y = 1 ; y <= HIHelix::kNTrackParameters ; ++y )
	    {
		  ...
		  corr1.fast( z, y ) =
		     ...
		     1. / sqrt( fabs( optimizedError.fast( z, z ) *
	    ...
	    cout
	       ...
	       << newDParams[4] << " " << endl
	       << "errors      "
	       << sqrt( fabs( optimizedError[0][0] ) ) << " "
	       << sqrt( fabs( optimizedError[1][1] ) ) << " "
	       << sqrt( fabs( optimizedError[2][2] ) ) << " "
	       << sqrt( fabs( optimizedError[3][3] ) ) << " "
	       << sqrt( fabs( optimizedError[4][4] ) ) << " " << endl
