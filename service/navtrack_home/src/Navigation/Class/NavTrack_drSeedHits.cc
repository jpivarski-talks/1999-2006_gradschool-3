...
// $Id: NavTrack_drSeedHits.cc,v 1.1 2000/04/05 15:43:03 marsh Exp $
//
// Revision history
//
// $Log: NavTrack_drSeedHits.cc,v $
...
NavTrack::seedDRHitLinks() const
{
   // If the cache is empty...
   if ( 0 == m_seedDRHitLinks )
   {
      // cast away const to fill cache from master block
      ((NavTrack*)this)->m_seedDRHitLinks =
         ((const NavTrackMaster&)masterBlock()).seedDRHitLinks(
            identifier() );
   }

   // return the object in the cache
   return m_seedDRHitLinks;
}

const NavTrack::DRHitTable*
NavTrack::seedDRHits() const
{
   // If the cache is empty...
   if ( 0 == m_seedDRHitTable )
   {
      // cast away const to fill cache from master block
      ((NavTrack*)this)->m_seedDRHitTable =
         ((const NavTrackMaster&)masterBlock()).seedDRHitTable( identifier() );
   }

   // return the object in the cache
   return m_seedDRHitTable;
}
