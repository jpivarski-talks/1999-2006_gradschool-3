...
// $Id: NavRich_calibratedHit.cc,v 1.1 2000/07/27 01:31:41 ts Exp $
//
// Revision history
//
// $Log: NavRich_calibratedHit.cc,v $
...
NavRich::RichCalibratedHitTable NavRich::richCalibratedHitPtrs( RichHypothesisIDType iHyp, const float minStdDev, const float maxStdDev ) const
{
  RichCalibratedHitTable pTable;
  ...
  for( RichPhotonTable::const_iterator item( photons.begin() ) ; item != tableEnd ; ++item ) {
               pTable.push_back( (const RichCalibratedHit*)(&*m_server.richCalibratedHit(m_server.richBump((*item)->bumpID())->padID())));
  } }
  return pTable;
}
