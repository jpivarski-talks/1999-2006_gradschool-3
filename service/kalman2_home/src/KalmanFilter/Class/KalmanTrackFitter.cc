...
// $Id: KalmanTrackFitter.cc,v 1.56 2002/03/22 00:50:17 wsun Exp $
//
// Revision history
//
...
//   intersector.setAttemptCurling( false ) ;
   ...
   for( int iHitDel = 0 ; iHitDel <= imax ; ++iHitDel )
   {
	 ...
	 << "NTUPLE Starting iteration "
	 << m_nHitsDropped + m_nHitsFlipped + 1
	 << " of track " << theSeed->identifier() << endl;
	 ...
	 report(DEBUG, kFacilityString)
	    << "Beginning to fit in the INWARD direction." << endl;
	 ...
	 report(DEBUG, kFacilityString)
	    << "Finished fitting in the INWARD direction." << endl;

      }
      else
      {
	 report(DEBUG, kFacilityString)
	    << "Beginning to fit in the OUTWARD direction." << endl;
	 ...
	 report(SYSTEM, kFacilityString)
	    << "NTUPLE sigz2 "
	    << aHelix->errorMatrix()(KTHelix::kZ0, KTHelix::kZ0)
	    << endl;

	 report(DEBUG, kFacilityString)
	    << "Finished fitting in the OUTWARD direction." << endl;
   ...
   const
{
   ...
   if ( aDirection != kInwardFit ) {
      int c_sign(0);
      if ( aHelix->curvature() > 0. ) c_sign = 1;
      if ( aHelix->curvature() < 0. ) c_sign = -1;

      double xdotp = ( aHelix->position().x() * aHelix->momentumUnit().x() +
		       aHelix->position().y() * aHelix->momentumUnit().y()   );
      ...
      int d_sign(0);
      if ( aHelix->d0() > 0. ) d_sign = 1;
      if ( aHelix->d0() < 0. ) d_sign = -1;

      KTHelix jims_dummy_helix(*aHelix);
      double jims_dummy(0.);
      jims_dummy_helix.moveToRadius(0.02, jims_dummy);
      double zat2cm = jims_dummy_helix.referencePoint().z();
      
      int z_sign(0);
      if ( zat2cm > 0. ) z_sign = 1;
      if ( zat2cm < 0. ) z_sign = -1;

      report(SYSTEM, kFacilityString)
	 << "NTUPLE " << " c_sign " << c_sign << " p_sign " << p_sign << " d_sign " << d_sign
	 << " curvature " << aHelix->curvature() << " xdotp " << xdotp << " d0 " << aHelix->d0()
	 << " zat2cm " << zat2cm << " z_sign " << z_sign << endl;
   }
