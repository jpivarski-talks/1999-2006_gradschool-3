...
//    int maxAttempts = 2 * m_surfaces.size() ;
   ...
   if ( m_printDiagnostics ) cout << "do..." << endl;
   do
   {
...
// 		 << " " << ( *m_currentSurface )->outerMaterial()->name()
	 ...
	 if( tryCurling && oldMomDotPos * newMomDotPos > 0. )
	 {
	    if ( m_printDiagnostics )
	       cout << "skipping out! (A) "
		    << tryCurling << " " << oldMomDotPos << " " << newMomDotPos << endl;

	    m_helix->revertToSnapshot( savedHelix ) ;
	    delete savedHelix ;

	    if ( m_printDiagnostics )
	       cout << "if( atEndOfSurfaces ) is if( " << atEndOfSurfaces << " )" << endl;
	 ...
	 {
	    if ( m_printDiagnostics )
	       cout << "inablock (B) " << m_checkCrossOrigin << " "
		    << oldMomDotPos << " " << newMomDotPos << " " << circleCenterRadius << " "
		    << ( *m_currentSurface )->radius() << " " << lastArc << " " << radialPosDiff << endl;
...
//	       return kHelixTransportError ;
	 ...
	 else
	 {
      ...
      {
	 if ( m_printDiagnostics ) cout << "this is a composite" << endl;
...
//	 m_helix->revertToSnapshot( savedHelix ) ;
   ...
   if ( m_printDiagnostics ) cout << "done whiling" << endl;
