...
// 
// Description: Subclass of TrackFitter for the C++ McCann filter.
//
// Usage:
//
...
// $Id: McCannTrackFitter.h,v 1.33 2003/07/25 23:30:57 mccann Exp $
//
// Revision history
//
// $Log: McCannTrackFitter.h,v $
...
// added accessors for McCannTrackFitter running options
//
...
class McCannTrackFitter : public TrackFitter
{
   ...
   public:
      ...
      McCannTrackFitter( HIFitConstraint* aConstraint = 0,
			 InputTrackInfo aTrackInfo = kNoHitsOnInputTrack ) ;
      virtual ~McCannTrackFitter();
	 ...
	 const STL_VECTOR(
      ...
      // the McCannHelixTransporter to move the track to the desired location.
	 ...
	 m_cathodeFittingWeightScale = aScale ;
      ...
      STL_VECTOR( McCannHelix* )* fittedHelices( HypoType aHypo ) const
   ...
   private:
      // ---------- Constructors and destructor ----------------
      McCannTrackFitter( const McCannTrackFitter& ) ; // stop default

      // ---------- assignment operator(s) ---------------------
      const McCannTrackFitter& operator=( const McCannTrackFitter& );
      // stop default
	 ...
	 const ;
      ...
      STL_VECTOR( McCannHelix* )* m_fittedHelices[ DBCandidate::kMaxHypo ] ;
      ...
      // hypothesis.  Therefore, the McCannTrackFitter needs to save these
      ...
      // Keep list of pointers to HIMSDedxOperations and McCannAddHitOperations
      // because they need to be reset when undoing the fit.
      STL_VECTOR( HIMSDedxOperation* )* m_msdedxOperations ;
      STL_VECTOR( McCannAddHitOperation* )* m_addHitOperations ;
      STL_VECTOR( McCannUpdateParamsOperation* )* m_updateParamsOperations ;
      STL_VECTOR( McCannUpdateParamsCondition* )* m_updateParamsConditions ;
...
//# include "McCannFilter/Template/McCannTrackFitter.cc"
//#endif
