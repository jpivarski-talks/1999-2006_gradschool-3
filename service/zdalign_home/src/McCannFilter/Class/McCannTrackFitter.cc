...
// $Id: McCannTrackFitter.cc,v 1.62 2003/10/15 19:11:33 mccann Exp $
//
// Revision history
//
// $Log: McCannTrackFitter.cc,v $
...
// Updated for use with McCannFilter producer.
//
...
#include "McCannFilter/McCannAddHitOperation.h"
//#include "McCannFilter/McCannFitQualityCondition.h"
#include "McCannFilter/McCannUpdateParamsCondition.h"
#include "McCannFilter/McCannUpdateZParamsCondition.h"
#include "McCannFilter/McCannUpdateParamsOperation.h"
...
static const char* const kFacilityString = "McCannTrackFitter" ;
...
//
McCannTrackFitter::McCannTrackFitter( HIFitConstraint* aConstraint,
   ...
   : m_printDiagnostics( false ),
     ...
     m_addHitOperations( new STL_VECTOR( McCannAddHitOperation* ) ),
     m_updateParamsOperations( new STL_VECTOR( McCannUpdateParamsOperation* )),
     m_updateParamsConditions( new STL_VECTOR( McCannUpdateParamsCondition* ) )
{
//    // TEMPORARY
//    m_fitConstraint = new HIMomentumConstraint ;
   ...
   {
      m_fittedHelices[ i ] = new STL_VECTOR( McCannHelix* ) ;
   }
...
// McCannTrackFitter::McCannTrackFitter( const McCannTrackFitter& rhs )
// {
...
McCannTrackFitter::~McCannTrackFitter()
{
   clearCache() ;
   for( int i = 0 ; i < DBCandidate::kMaxHypo ; ++i )
   {
...
void
McCannTrackFitter::clearCache()
{
   // Delete the helices in the vectors.
   for( int i = 0 ; i < DBCandidate::kMaxHypo ; ++i )
   {
      STL_VECTOR( McCannHelix* )::iterator helixItr =
	 m_fittedHelices[ i ]->begin() ;
      STL_VECTOR( McCannHelix* )::iterator helixEnd =
...
//
// const McCannTrackFitter& McCannTrackFitter::operator=(
//    const McCannTrackFitter& rhs )
// {
//   if( this != &rhs ) {
...
void
McCannTrackFitter::setHyposToFit( STL_VECTOR( DBCandidate::Hypo )& aVector )
{
   *m_hyposToFit = aVector ;
}

void
McCannTrackFitter::makeConditionsAndOperations() 
{
   ...
   McCannAddHitOperation* addHit0 = new McCannAddHitOperation( m_hitChisqCut );

   m_fitOperation->addChild( addHit0 ) ;
   m_addHitOperations->push_back( addHit0 ) ;

   McCannUpdateParamsCondition* updateCondition0 =
      new McCannUpdateParamsCondition ;
   McCannUpdateParamsOperation* update0 =
      new McCannUpdateParamsOperation(
	 updateCondition0,
	 new McCannUpdateZParamsCondition( m_updateErr2Z0,
   ...
   McCannAddHitOperation* addHit1 = new McCannAddHitOperation( m_hitChisqCut );
   ...
   McCannUpdateParamsCondition* updateCondition1 =
      new McCannUpdateParamsCondition ;
   McCannUpdateParamsOperation* update1 =
      new McCannUpdateParamsOperation(
	 updateCondition1,
	 new McCannUpdateZParamsCondition( m_updateErr2Z0,
					   m_updateErr2CotTheta,
...
void
McCannTrackFitter::clearConditionsAndOperations()
{
...
DABoolean
McCannTrackFitter::fitTrack( const TRSeedTrack* theSeed,
			     Frame& theFrame,
			     DBCandidate::Hypo aHypo )
{
   // Check that this track and hypothesis has not been fit already.
   STL_VECTOR( McCannHelix* )::iterator itr = m_fittedHelices[aHypo]->begin();
   STL_VECTOR( McCannHelix* )::iterator end = m_fittedHelices[aHypo]->end() ;
   ...
   McCannHelix* fittedHelix =
      ( McCannHelix* ) fittedTrack( theSeed,
				    theFrame,
...
KTHelix*
McCannTrackFitter::fittedTrack( const TRSeedTrack* theSeed,
				Frame& theFrame,
				DBCandidate::Hypo aHypo )
{
...
KTHelix*
McCannTrackFitter::removeHitsFromFit(
   ...
   GeV aMass )
{
   ...
   McCannHelix* aHelix = new McCannHelix( *theSeed, aMass ) ;
   McCannHelix copyOfOriginal = *aHelix ;
   ...
   STL_VECTOR( McCannAddHitOperation* )::const_iterator addHitItr =
      m_addHitOperations->begin() ;
   STL_VECTOR( McCannAddHitOperation* )::const_iterator addHitEnd =
   ...
   STL_VECTOR( McCannUpdateParamsCondition* )::const_iterator updateCondItr =
      m_updateParamsConditions->begin() ;
   STL_VECTOR( McCannUpdateParamsCondition* )::const_iterator updateCondEnd =
   ...
   DABoolean aExtractHitsIfNull )
{
...
#endif
   {
      if( m_inwardFit == 0 )
      {
	 m_inwardFit = new McCannHelix( *theSeed, aMass ) ;
      }
...
//   intersector.setAttemptCurling( false ) ;
   ...
   if( !m_keepPreviousHitList )
   {
      // Reset quantities for the McCann quality object.
      m_nHitsDropped = 0 ;
      m_nHitsFlipped = 0 ;
   ...
   for( int iHitDel = 0 ; iHitDel <= imax ; ++iHitDel )
   {
	       ...
	       !intersector.setCurrentSurface( secondOutermostHitSurface ) ) )
	 {
	    if( m_inwardFit == 0 )
	    {
	       m_inwardFit = new McCannHelix( copyOfOriginal ) ;
	    }
	    else
	    {
	 ...
	 if( m_inwardFit == 0 )
	 {
	    m_inwardFit = new McCannHelix( *aHelix ) ;
	 }
	 else
	 {
	    *m_inwardFit = *aHelix ;
	 ...
	 // Return aHelix to original and non-McCann transport it to the
...
//       if( iHitDel < imax )
	 ...
	 if( worstHit != 0 && maxPull > m_maxHitPull )
	 {
	    ...
	    {
	       // To be stored in McCann quality object.
	       m_moreBadHits = true ;
...
DABoolean
McCannTrackFitter::fitTrackOneDirection( HIHelixIntersector& aIntersector,
					 McCannHelix* aHelix,
					 FitDirection aDirection,
   ...
   const
{
   ...
   McCannHelix lastHitHelix( *aHelix ) ;
   int nHitsOnTrack = 0 ;
   int lastNHits = 0 ;
...
void
McCannTrackFitter::generateSiSurfaceCache( Frame& theFrame )
{
...
void
McCannTrackFitter::generateIRSurfaceCache( Frame& theFrame )
{
...
void
McCannTrackFitter::generateDRSurfaceCache( Frame& theFrame )
{
...
void
McCannTrackFitter::generateZDSurfaceCache( Frame& theFrame )
{
   FAItem<DetectorConfiguration>                       iConfig   ;
   extract( theFrame.record( Stream::kBaseGeometry ) , iConfig ) ;
...
McCannTrackFitter::generateSurfaces(
   const TRSeedTrack* theSeed,
   Frame& theFrame,
   const STL_VECTOR(
      HIDRSurfaceFactory::DRHitAndDriftDistance )* aDRVector,
...
void
McCannTrackFitter::generateHitSurfaces(
   const TRSeedTrack* theSeed,
   Frame& theFrame,
   const STL_VECTOR(
      HIDRSurfaceFactory::DRHitAndDriftDistance )* aDRVector,
...
//
DABoolean
McCannTrackFitter::event( Frame& theFrame ) const
{
   // Cast away the const on *this.
   McCannTrackFitter* thisNonConst = const_cast< McCannTrackFitter* >( this ) ;
...
DABoolean McCannTrackFitter::event( Frame& theFrame, HypoType aHypo ) const
{
   DABoolean success = TrackFitter::event( theFrame, aHypo ) ;

   McCannTrackFitter* thisNonConst =
      const_cast< McCannTrackFitter* >( this ) ;
