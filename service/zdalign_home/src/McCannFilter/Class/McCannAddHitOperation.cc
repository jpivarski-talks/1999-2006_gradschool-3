...
// $Id: McCannAddHitOperation.cc,v 1.24 2003/10/15 19:11:33 mccann Exp $
//
// Revision history
//
// $Log: McCannAddHitOperation.cc,v $
...
// * Moved track parameter update to McCannUpdateParamsOperation.
//
...
"McCannFilter.McCannAddHitOperation" ;
...
//
McCannAddHitOperation::McCannAddHitOperation( double aMaxHitChisq )
   : m_maxHitChisq( aMaxHitChisq ),
     m_removeHitsFromFit( false )
{
}

// McCannAddHitOperation::McCannAddHitOperation( const McCannAddHitOperation& rhs )
// {
...
McCannAddHitOperation::~McCannAddHitOperation()
{
}
...
//
// const McCannAddHitOperation& McCannAddHitOperation::operator=( const McCannAddHitOperation& rhs )
// {
//   if( this != &rhs ) {
   ...
   const HIIntersectionSurface& aLastSurface ) const
{
   ...
   if( aHelixAfterOperation.helixType() == McCannHelix::kHelixType )
   {
      kalmanHelix = ( McCannHelix* ) &aHelixAfterOperation ;
   }
   else
   {
   ...
   STL_VECTOR( McCannHelix::HitAndLinkData ) hitsToAddToTrack ;
...
//       }
	 ...
	 hitsAdded = true ;
      ...
      McCannHelix::HitAndLinkData newHitAndLinkData(
...
void
McCannAddHitOperation::invert5x5SymMatrix( const HepSymMatrix& input,
					   HepSymMatrix& output )
{
   double tmp1[4][5] ;
   double tmp2[5][5] ;
