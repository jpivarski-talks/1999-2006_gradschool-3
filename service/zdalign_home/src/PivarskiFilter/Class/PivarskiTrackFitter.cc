...
// $Id: PivarskiTrackFitter.cc,v 1.62 2003/10/15 19:11:33 mccann Exp $
//
// Revision history
//
// $Log: PivarskiTrackFitter.cc,v $
// Revision 1.62  2003/10/15 19:11:33  mccann
// Wrapped excessive DEBUG printouts with m_printDiagnostics==true requirement
//
// Revision 1.61  2003/07/28 15:01:21  mccann
// fitter now checks DetectorConfiguration before asking for SI/ZD objects
//
// Revision 1.60  2003/07/25 23:30:54  mccann
// Added the ZD to track fitting
//
// Revision 1.59  2003/04/23 16:01:42  cdj
// fixed double deletion bug by creating new class, KSurfaceVector, to manage the memory
//
// Revision 1.58  2003/04/17 17:01:38  bkh
// Add ZD options
//
...
// Updated for use with PivarskiFilter producer.
//
...
#include "PivarskiFilter/PivarskiAddHitOperation.h"
//#include "PivarskiFilter/PivarskiFitQualityCondition.h"
#include "PivarskiFilter/PivarskiUpdateParamsCondition.h"
#include "PivarskiFilter/PivarskiUpdateZParamsCondition.h"
#include "PivarskiFilter/PivarskiUpdateParamsOperation.h"
...
static const char* const kFacilityString = "PivarskiTrackFitter" ;
...
//
PivarskiTrackFitter::PivarskiTrackFitter( HIFitConstraint* aConstraint,
   ...
   : m_printDiagnostics( false ),
     ...
     m_addHitOperations( new STL_VECTOR( PivarskiAddHitOperation* ) ),
     m_updateParamsOperations( new STL_VECTOR( PivarskiUpdateParamsOperation* )),
     m_updateParamsConditions( new STL_VECTOR( PivarskiUpdateParamsCondition* ) )
{
//    // TEMPORARY
//    m_fitConstraint = new HIMomentumConstraint ;
   ...
   {
      m_fittedHelices[ i ] = new STL_VECTOR( PivarskiHelix* ) ;
   }
...
// PivarskiTrackFitter::PivarskiTrackFitter( const PivarskiTrackFitter& rhs )
// {
...
PivarskiTrackFitter::~PivarskiTrackFitter()
{
   clearCache() ;
   for( int i = 0 ; i < DBCandidate::kMaxHypo ; ++i )
   {
...
void
PivarskiTrackFitter::clearCache()
{
   // Delete the helices in the vectors.
   for( int i = 0 ; i < DBCandidate::kMaxHypo ; ++i )
   {
      STL_VECTOR( PivarskiHelix* )::iterator helixItr =
	 m_fittedHelices[ i ]->begin() ;
      STL_VECTOR( PivarskiHelix* )::iterator helixEnd =
...
//
// const PivarskiTrackFitter& PivarskiTrackFitter::operator=(
//    const PivarskiTrackFitter& rhs )
// {
//   if( this != &rhs ) {
...
void
PivarskiTrackFitter::setHyposToFit( STL_VECTOR( DBCandidate::Hypo )& aVector )
{
   *m_hyposToFit = aVector ;
}

void
PivarskiTrackFitter::makeConditionsAndOperations() 
{
   ...
   PivarskiAddHitOperation* addHit0 = new PivarskiAddHitOperation( m_hitChisqCut );

   m_fitOperation->addChild( addHit0 ) ;
   m_addHitOperations->push_back( addHit0 ) ;

   PivarskiUpdateParamsCondition* updateCondition0 =
      new PivarskiUpdateParamsCondition ;
   PivarskiUpdateParamsOperation* update0 =
      new PivarskiUpdateParamsOperation(
	 updateCondition0,
	 new PivarskiUpdateZParamsCondition( m_updateErr2Z0,
   ...
   PivarskiAddHitOperation* addHit1 = new PivarskiAddHitOperation( m_hitChisqCut );
   ...
   PivarskiUpdateParamsCondition* updateCondition1 =
      new PivarskiUpdateParamsCondition ;
   PivarskiUpdateParamsOperation* update1 =
      new PivarskiUpdateParamsOperation(
	 updateCondition1,
	 new PivarskiUpdateZParamsCondition( m_updateErr2Z0,
					   m_updateErr2CotTheta,
...
void
PivarskiTrackFitter::clearConditionsAndOperations()
{
...
DABoolean
PivarskiTrackFitter::fitTrack( const TRSeedTrack* theSeed,
			     Frame& theFrame,
			     DBCandidate::Hypo aHypo )
{
   // Check that this track and hypothesis has not been fit already.
   STL_VECTOR( PivarskiHelix* )::iterator itr = m_fittedHelices[aHypo]->begin();
   STL_VECTOR( PivarskiHelix* )::iterator end = m_fittedHelices[aHypo]->end() ;
   ...
   PivarskiHelix* fittedHelix =
      ( PivarskiHelix* ) fittedTrack( theSeed,
				    theFrame,
...
KTHelix*
PivarskiTrackFitter::fittedTrack( const TRSeedTrack* theSeed,
				Frame& theFrame,
				DBCandidate::Hypo aHypo )
{
...
KTHelix*
PivarskiTrackFitter::fittedTrack(
...
KTHelix*
PivarskiTrackFitter::removeHitsFromFit(
   ...
   GeV aMass )
{
   ...
   PivarskiHelix* aHelix = new PivarskiHelix( *aHelix, aMass ) ;
   PivarskiHelix copyOfOriginal = *aHelix ;
   ...
   STL_VECTOR( PivarskiAddHitOperation* )::const_iterator addHitItr =
      m_addHitOperations->begin() ;
   STL_VECTOR( PivarskiAddHitOperation* )::const_iterator addHitEnd =
   ...
   STL_VECTOR( PivarskiUpdateParamsCondition* )::const_iterator updateCondItr =
      m_updateParamsConditions->begin() ;
   STL_VECTOR( PivarskiUpdateParamsCondition* )::const_iterator updateCondEnd =
   ...
   PivarskiHelix lastHelix( *aHelix ) ;
   ...
   DABoolean aExtractHitsIfNull )
{
   ...
   // Make a non-const copy of the seed track.
   PivarskiHelix* aHelix = new PivarskiHelix( *theSeed, aMass ) ;
   PivarskiHelix copyOfOriginal = *aHelix ;
...
#endif
   {
      if( m_inwardFit == 0 )
      {
	 m_inwardFit = new PivarskiHelix( *theSeed, aMass ) ;
      }
...
//   intersector.setAttemptCurling( false ) ;
   ...
   if( !m_keepPreviousHitList )
   {
      // Reset quantities for the Pivarski quality object.
      m_nHitsDropped = 0 ;
      m_nHitsFlipped = 0 ;
   ...
   for( int iHitDel = 0 ; iHitDel <= imax ; ++iHitDel )
   {
	       ...
	       !intersector.setCurrentSurface( secondOutermostHitSurface ) ) )
	 {
	    if( m_inwardFit == 0 )
	    {
	       m_inwardFit = new PivarskiHelix( copyOfOriginal ) ;
	    }
	    else
	    {
	 ...
	 if( m_inwardFit == 0 )
	 {
	    m_inwardFit = new PivarskiHelix( *aHelix ) ;
	 }
	 else
	 {
	    *m_inwardFit = *aHelix ;
	 ...
	 // Return aHelix to original and non-Pivarski transport it to the
...
//       if( iHitDel < imax )
	 ...
	 if( worstHit != 0 && maxPull > m_maxHitPull )
	 {
	    ...
	    {
	       // To be stored in Pivarski quality object.
	       m_moreBadHits = true ;
...
DABoolean
PivarskiTrackFitter::fitTrackOneDirection( HIHelixIntersector& aIntersector,
					 PivarskiHelix* aHelix,
					 FitDirection aDirection,
   ...
   const
{
   ...
   PivarskiHelix lastHitHelix( *aHelix ) ;
   int nHitsOnTrack = 0 ;
   int lastNHits = 0 ;
...
void
PivarskiTrackFitter::generateSiSurfaceCache( Frame& theFrame )
{
   FAItem<DetectorConfiguration>                       iConfig   ;
   extract( theFrame.record( Stream::kBaseGeometry ) , iConfig ) ;
...
void
PivarskiTrackFitter::generateIRSurfaceCache( Frame& theFrame )
{
...
void
PivarskiTrackFitter::generateDRSurfaceCache( Frame& theFrame )
{
...
void
PivarskiTrackFitter::generateZDSurfaceCache( Frame& theFrame )
{
   FAItem<DetectorConfiguration>                       iConfig   ;
   extract( theFrame.record( Stream::kBaseGeometry ) , iConfig ) ;
   ...
   // Generate the cache of DR surfaces if necessary.
   if( m_zdCacheFilled || m_noFitOperation->numberOfChildren() == 0 )
   {
      return ;
   }

   // First, clear the existing cache.
   m_zdCache.clearSurfaces();
...
PivarskiTrackFitter::generateSurfaces(
   const TRSeedTrack* theSeed,
   Frame& theFrame,
   const STL_VECTOR(
      HIDRSurfaceFactory::DRHitAndDriftDistance )* aDRVector,
   ...
   DABoolean aExtractHitsIfNull )
{
   ...
   if ( detectorConfig->useZDnotSI() ) {
      generateZDSurfaceCache( theFrame ) ;
   } else {
...
void
PivarskiTrackFitter::generateHitSurfaces(
   const TRSeedTrack* theSeed,
   Frame& theFrame,
   const STL_VECTOR(
      HIDRSurfaceFactory::DRHitAndDriftDistance )* aDRVector,
...
//
DABoolean
PivarskiTrackFitter::event( Frame& theFrame ) const
{
   // Cast away the const on *this.
   PivarskiTrackFitter* thisNonConst = const_cast< PivarskiTrackFitter* >( this ) ;
...
DABoolean PivarskiTrackFitter::event( Frame& theFrame, HypoType aHypo ) const
{
   DABoolean success = TrackFitter::event( theFrame, aHypo ) ;

   PivarskiTrackFitter* thisNonConst =
      const_cast< PivarskiTrackFitter* >( this ) ;
