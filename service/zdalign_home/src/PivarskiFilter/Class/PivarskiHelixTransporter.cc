...
// Module:      PivarskiHelixTransporter
// 
...
// $Id: PivarskiHelixTransporter.cc,v 1.3 2003/09/10 21:31:56 wsun Exp $
//
// Revision history
//
// $Log: PivarskiHelixTransporter.cc,v $
// Revision 1.3  2003/09/10 21:31:56  wsun
// Added ZD surfaces and check of DetectorConfiguration.
//
...
#include "PivarskiFilter/PivarskiHelixTransporter.h"
...
static const char* const kFacilityString = "PivarskiFilter.PivarskiHelixTransporter" ;

// ---- cvs-based strings (Id and Tag with which file was checked out)
static const char* const kIdString  = "$Id: PivarskiHelixTransporter.cc,v 1.3 2003/09/10 21:31:56 wsun Exp $";
...
//
PivarskiHelixTransporter::PivarskiHelixTransporter( Frame& aFrame )
   : m_surfacesOK( true ),
...
// PivarskiHelixTransporter::PivarskiHelixTransporter( const PivarskiHelixTransporter& rhs )
// {
...
PivarskiHelixTransporter::~PivarskiHelixTransporter()
{
...
//
// const PivarskiHelixTransporter& PivarskiHelixTransporter::operator=( const PivarskiHelixTransporter& rhs )
// {
//   if( this != &rhs ) {
...
DABoolean
PivarskiHelixTransporter::transportThroughMaterial(
...
void
PivarskiHelixTransporter::generateSurfaces( Frame& theFrame )
{
   ...
   // Generate Si or ZD surfaces, depending on detector configuration.
   FAItem< DetectorConfiguration > config ;
   extract( theFrame.record( Stream::kBaseGeometry ), config ) ;

   if( config->useZDnotSI() )
   {
      HIZDSurfaceFactory zdFactory( theFrame ) ;
      if( zdFactory.geometryAvailable() )
      {
	 zdFactory.generateTubeCylinders( *m_surfaces, &m_msdedxOperation ) ;
	 zdFactory.generateSurfacesWithoutHits( *m_surfaces,
						&m_msdedxOperation );
      }
      else
      {
