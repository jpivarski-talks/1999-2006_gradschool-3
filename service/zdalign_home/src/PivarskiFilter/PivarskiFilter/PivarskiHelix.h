...
// $Id: PivarskiHelix.h,v 1.18 2002/04/27 17:42:39 cleo3 Exp $
//
// Revision history
//
// $Log: PivarskiHelix.h,v $
...
class PivarskiHelix : public HIFitHelix
{
   ...
   public:
      ...
      PivarskiHelix();
      //PivarskiHelix( const PivarskiHelix& );
      PivarskiHelix( const KTHelix& aHelix,
      ...
      //const PivarskiHelix& operator=( const PivarskiHelix& );

      //virtual ~PivarskiHelix();
      ...
      virtual HIHelix* newSnapshot() { return new PivarskiHelix( *this ) ; }
      virtual void revertToSnapshot( const HIHelix* aSnapshot )
      {
	 if( aSnapshot->helixType() == helixType() )
	    *this = *( ( PivarskiHelix* )aSnapshot ) ;
      }
	 ...
	 m_transportNewErrorMatrix = true ;
      ...
      // The Pivarski filter is allowed to throw away only so many hits.
	 ...
	 const HepVector& aTransportVector ) ;
      ...
      { return PivarskiHelix::kHelixType ; }
...
//# include "PivarskiFilter/Template/HIFitHelix.cc"
//#endif
