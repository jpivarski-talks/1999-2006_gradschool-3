...
// 
// Description: Subclass of TrackFitter for the C++ Pivarski filter.
//
// Usage:
//
...
// $Id: PivarskiTrackFitter.h,v 1.33 2003/07/25 23:30:57 mccann Exp $
//
// Revision history
//
// $Log: PivarskiTrackFitter.h,v $
// Revision 1.33  2003/07/25 23:30:57  mccann
// Added the ZD to track fitting
//
// Revision 1.32  2003/04/23 16:01:46  cdj
// fixed double deletion bug by creating new class, KSurfaceVector, to manage the memory
//
// Revision 1.31  2003/04/17 17:01:41  bkh
// Add ZD options
//
...
// added accessors for PivarskiTrackFitter running options
//
...
#include "PivarskiFilter/KSurfaceVector.h"
...
class PivarskiTrackFitter : public TrackFitter
{
   ...
   public:
      ...
      PivarskiTrackFitter( HIFitConstraint* aConstraint = 0,
			 InputTrackInfo aTrackInfo = kNoHitsOnInputTrack ) ;
      virtual ~PivarskiTrackFitter();
	 ...
	 const STL_VECTOR(
      ...
      // the PivarskiHelixTransporter to move the track to the desired location.
	 ...
	 const STL_VECTOR(
      ...
      void updateZDSurfaceCache() { m_zdCacheFilled = false ; }
	 ...
	 m_cathodeFittingWeightScale = aScale ;
      ...
      STL_VECTOR( PivarskiHelix* )* fittedHelices( HypoType aHypo ) const
   ...
   private:
      // ---------- Constructors and destructor ----------------
      PivarskiTrackFitter( const PivarskiTrackFitter& ) ; // stop default

      // ---------- assignment operator(s) ---------------------
      const PivarskiTrackFitter& operator=( const PivarskiTrackFitter& );
      // stop default
      ...
      void generateZDSurfaceCache( Frame& ) ;
	 ...
	 const ;
      ...
      STL_VECTOR( PivarskiHelix* )* m_fittedHelices[ DBCandidate::kMaxHypo ] ;
      ...
      KSurfaceVector m_siCache ;
      DABoolean m_irCacheFilled ;
      KSurfaceVector m_irCache ;
      DABoolean m_drCacheFilled ;
      KSurfaceVector m_drCache ;
      DABoolean m_zdCacheFilled ;
      KSurfaceVector m_zdCache ;
      DABoolean m_hitSurfacesFilled ;
      KSurfaceVector m_hitSurfaces ;
      ...
      // hypothesis.  Therefore, the PivarskiTrackFitter needs to save these
      ...
      // Keep list of pointers to HIMSDedxOperations and PivarskiAddHitOperations
      // because they need to be reset when undoing the fit.
      STL_VECTOR( HIMSDedxOperation* )* m_msdedxOperations ;
      STL_VECTOR( PivarskiAddHitOperation* )* m_addHitOperations ;
      STL_VECTOR( PivarskiUpdateParamsOperation* )* m_updateParamsOperations ;
      STL_VECTOR( PivarskiUpdateParamsCondition* )* m_updateParamsConditions ;
...
//# include "PivarskiFilter/Template/PivarskiTrackFitter.cc"
//#endif
