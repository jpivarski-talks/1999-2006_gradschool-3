...
// $Id: PivarskiTrackFitterProxy.cc,v 1.19 2003/09/10 20:59:46 wsun Exp $
//
// Revision history
//
// $Log: PivarskiTrackFitterProxy.cc,v $
// Revision 1.19  2003/09/10 20:59:46  wsun
// Replaced updateXXGeom() functions bound to geometry streams with checks of
// Record::cacheIdentifier() in faultHandler."
//
// Revision 1.18  2003/09/09 23:51:16  wsun
// Added updateZDGeom().
//
// Revision 1.17  2003/07/25 23:34:50  mccann
...
//
static const char* const kFacilityString = "PivarskiTrackFitterProxy" ;

// ---- cvs-based strings (Id and Tag with which file was checked out)
static const char* const kIdString  = "$Id: PivarskiTrackFitterProxy.cc,v 1.19 2003/09/10 20:59:46 wsun Exp $";
...
PivarskiTrackFitterProxy::PivarskiTrackFitterProxy( PivarskiProd* aPivarskiProd )
   : m_kalmanTrackFitter( 0 ),
     m_kalmanProd( aPivarskiProd ),
     m_siCacheIdentifier( 0 ),
     m_irCacheIdentifier( 0 ),
     m_drCacheIdentifier( 0 ),
     m_zdCacheIdentifier( 0 )
{
   if( m_kalmanProd != 0 &&
       m_kalmanProd->bfieldMagnitude() != 0. ) // default value from PivarskiProd
   {
      // This will override the MagneticField object in the Frame.
      HIMagField::instance()->setBfield( m_kalmanProd->bfieldMagnitude() ) ;
   }

   bind( &PivarskiTrackFitterProxy::updateMagField, Stream::kStartRun ) ;
//    bind( &PivarskiTrackFitterProxy::updateSiGeom,   Stream::kSVAlignment ) ;
//    bind( &PivarskiTrackFitterProxy::updateIRGeom,   Stream::kBaseGeometry ) ;
//    bind( &PivarskiTrackFitterProxy::updateDRGeom,   Stream::kDRAlignment ) ;
//    bind( &PivarskiTrackFitterProxy::updateZDGeom,   Stream::kZDAlignment ) ;
}

// PivarskiTrackFitterProxy::PivarskiTrackFitterProxy( const PivarskiTrackFitterProxy& iproxy )
// {
//   *this = iproxy;
// }

PivarskiTrackFitterProxy::~PivarskiTrackFitterProxy()
{
...
//
// const PivarskiTrackFitterProxy& PivarskiTrackFitterProxy::operator=( const PivarskiTrackFitterProxy& iproxy )
// {
//   if( this != &iproxy ) {
...
//
void
PivarskiTrackFitterProxy::invalidateCache()
{
...
const PivarskiTrackFitterProxy::value_type*
PivarskiTrackFitterProxy::faultHandler( const Record& iRecord,
				      const DataKey& iKey )
{
...
//   assert( 0 == m_kalmanTrackFitter );
   ...
   if( 0 == m_kalmanTrackFitter )
   {
      ...
      m_kalmanTrackFitter = new PivarskiTrackFitter ;
   }
   else
   {
      ...
      {
	 if( m_zdCacheIdentifier !=
	     iRecord.frame().record( Stream::kZDAlignment ).cacheIdentifier() )
	 {
	    m_kalmanTrackFitter->updateZDSurfaceCache() ;
	 }
      }
      else
      {
	 if( m_siCacheIdentifier !=
	     iRecord.frame().record( Stream::kSVAlignment ).cacheIdentifier() )
	 {
	    m_kalmanTrackFitter->updateSiSurfaceCache() ;
	 }
      }

      if( m_irCacheIdentifier !=
	  iRecord.frame().record( Stream::kBaseGeometry ).cacheIdentifier() )
      {
	 m_kalmanTrackFitter->updateIRSurfaceCache() ;
      }

      if( m_drCacheIdentifier !=
	  iRecord.frame().record( Stream::kDRAlignment ).cacheIdentifier() )
      {
   ...
   if( iConfig->useZDnotSI() )
   {
      m_zdCacheIdentifier = iRecord.frame().record(
	 Stream::kZDAlignment ).cacheIdentifier() ;
   }
   else
   {
      m_siCacheIdentifier = iRecord.frame().record(
	 Stream::kSVAlignment ).cacheIdentifier() ;
   }
   m_irCacheIdentifier = iRecord.frame().record(
      Stream::kBaseGeometry ).cacheIdentifier() ;
   m_drCacheIdentifier = iRecord.frame().record(
      Stream::kDRAlignment ).cacheIdentifier() ;
   ...
   // If the vector from PivarskiProd is empty, then fit all hypos.
   ...
   if( m_kalmanProd->optimizeForAllHypos() )
   {
      ...
      // them, and fits each track under each hypothesis.  These PivarskiHelix
      // objects are kept as data members of PivarskiTrackFitter.  This
...
//
void
PivarskiTrackFitterProxy::updateMagField( const Record& iRecord )
{
   HIMagField::instance()->setBfield( iRecord ) ;
}

// void
// PivarskiTrackFitterProxy::updateSiGeom( const Record& iRecord )
// {
//    if( m_kalmanTrackFitter != 0 )
//    {
//       m_kalmanTrackFitter->updateSiSurfaceCache() ;
...
// void
// PivarskiTrackFitterProxy::updateIRGeom( const Record& iRecord )
// {
//    if( m_kalmanTrackFitter != 0 )
//    {
//       m_kalmanTrackFitter->updateIRSurfaceCache() ;
...
// void
// PivarskiTrackFitterProxy::updateDRGeom( const Record& iRecord )
// {
//    if( m_kalmanTrackFitter != 0 )
//    {
//       m_kalmanTrackFitter->updateDRSurfaceCache() ;
...
// void
// PivarskiTrackFitterProxy::updateZDGeom( const Record& iRecord )
// {
//    if( m_kalmanTrackFitter != 0 )
//    {
//       m_kalmanTrackFitter->updateZDSurfaceCache() ;
...
//void
//PivarskiTrackFitterProxy::boundMethod( const Record& iRecord )
//{
...
//-----------------------------------------------
// DO NOT DELETE
PROXY_BIND_METHOD( PivarskiTrackFitterProxy )
//-----------------------------------------------
