...
// $Id: McCannTrackFitterProxy.cc,v 1.19 2003/09/10 20:59:46 wsun Exp $
//
// Revision history
//
// $Log: McCannTrackFitterProxy.cc,v $
...
//
static const char* const kFacilityString = "McCannTrackFitterProxy" ;

// ---- cvs-based strings (Id and Tag with which file was checked out)
static const char* const kIdString  = "$Id: McCannTrackFitterProxy.cc,v 1.19 2003/09/10 20:59:46 wsun Exp $";
...
McCannTrackFitterProxy::McCannTrackFitterProxy( McCannProd* aMcCannProd )
   : m_kalmanTrackFitter( 0 ),
     m_kalmanProd( aMcCannProd ),
...
{
   if( m_kalmanProd != 0 &&
       m_kalmanProd->bfieldMagnitude() != 0. ) // default value from McCannProd
   {
      // This will override the MagneticField object in the Frame.
      HIMagField::instance()->setBfield( m_kalmanProd->bfieldMagnitude() ) ;
   }

   bind( &McCannTrackFitterProxy::updateMagField, Stream::kStartRun ) ;
//    bind( &McCannTrackFitterProxy::updateSiGeom,   Stream::kSVAlignment ) ;
//    bind( &McCannTrackFitterProxy::updateIRGeom,   Stream::kBaseGeometry ) ;
//    bind( &McCannTrackFitterProxy::updateDRGeom,   Stream::kDRAlignment ) ;
//    bind( &McCannTrackFitterProxy::updateZDGeom,   Stream::kZDAlignment ) ;
}

// McCannTrackFitterProxy::McCannTrackFitterProxy( const McCannTrackFitterProxy& iproxy )
// {
//   *this = iproxy;
// }

McCannTrackFitterProxy::~McCannTrackFitterProxy()
{
...
//
// const McCannTrackFitterProxy& McCannTrackFitterProxy::operator=( const McCannTrackFitterProxy& iproxy )
// {
//   if( this != &iproxy ) {
...
//
void
McCannTrackFitterProxy::invalidateCache()
{
...
const McCannTrackFitterProxy::value_type*
McCannTrackFitterProxy::faultHandler( const Record& iRecord,
				      const DataKey& iKey )
{
...
//   assert( 0 == m_kalmanTrackFitter );
   ...
   if( 0 == m_kalmanTrackFitter )
   {
      ...
      m_kalmanTrackFitter = new McCannTrackFitter ;
   }
   else
   {
   ...
   // If the vector from McCannProd is empty, then fit all hypos.
   ...
   if( m_kalmanProd->optimizeForAllHypos() )
   {
      ...
      // them, and fits each track under each hypothesis.  These McCannHelix
      // objects are kept as data members of McCannTrackFitter.  This
...
//
void
McCannTrackFitterProxy::updateMagField( const Record& iRecord )
{
   HIMagField::instance()->setBfield( iRecord ) ;
}

// void
// McCannTrackFitterProxy::updateSiGeom( const Record& iRecord )
// {
//    if( m_kalmanTrackFitter != 0 )
//    {
//       m_kalmanTrackFitter->updateSiSurfaceCache() ;
...
// void
// McCannTrackFitterProxy::updateIRGeom( const Record& iRecord )
// {
//    if( m_kalmanTrackFitter != 0 )
//    {
//       m_kalmanTrackFitter->updateIRSurfaceCache() ;
...
// void
// McCannTrackFitterProxy::updateDRGeom( const Record& iRecord )
// {
//    if( m_kalmanTrackFitter != 0 )
//    {
//       m_kalmanTrackFitter->updateDRSurfaceCache() ;
...
// void
// McCannTrackFitterProxy::updateZDGeom( const Record& iRecord )
// {
//    if( m_kalmanTrackFitter != 0 )
//    {
//       m_kalmanTrackFitter->updateZDSurfaceCache() ;
...
//void
//McCannTrackFitterProxy::boundMethod( const Record& iRecord )
//{
...
//-----------------------------------------------
// DO NOT DELETE
PROXY_BIND_METHOD( McCannTrackFitterProxy )
//-----------------------------------------------
