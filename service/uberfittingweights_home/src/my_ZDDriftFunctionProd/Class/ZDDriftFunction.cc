...
// $Id: ZDDriftFunction.cc,v 1.2 2003/10/10 17:36:39 mccann Exp $
//
// Revision history
//
   ...
   m_ZDDriftFunctionProd( ZDDriftFunctionProd )
{
  ...
  //
     ...
     const double kFavoriteNumber = -2648.0126;
     ...
     DABoolean gotDriftMin(false), gotDriftMax(false), gotEntAngMin(false), gotEntAngMax(false);
	...
	for (int l = 0;  l < kmaxZDlayer;  l++) {
     ...
     DABoolean gotThetaStdCor(false), gotThetaMin(false), gotThetaMax(false);
	...
	if (true) { // to protect the "int l" declaration on some compilers
     ...
     DABoolean gotPhiStdCor(false), gotPhiMax(false);
	...
	if (true) { // to protect the "int l" declaration on some compilers
     ...
     double sbDriftMin(sbDrift(0.)), sbDriftMax(sbDrift(1.));
