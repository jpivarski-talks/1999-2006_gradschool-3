...
// $Id: DataDriftFunction.cc,v 1.20 2003/10/03 12:54:50 dubrovin Exp $
//
// Revision history
//
// $Log: DataDriftFunction.cc,v $
// Revision 1.20  2003/10/03 12:54:50  dubrovin
// Add new implementation of the Entrance angle correction
//
...
////Constants for alternative (to Garfield) drift distance correction see *.h
const float  DataDriftFunction::kCorr_TimeMaxA   =    500000.0 ;
const float  DataDriftFunction::kCorr_EAngleMaxA =        90.0 ; 
const int    DataDriftFunction::kCorr_FileSizeA  =        1924 ; 
//----------------------------------------------------------------------------
	  ...
	  drIII_df_data(kmaxDFside,STL_VECTOR(Singlelayer_df_data)(kmaxDRlayer))
{
  ...
  */  
  ...
  //-------------------------------------------------------------------------------------
  //
  // 2003/09/25, D.M.: Initialize/Unpack NEW Drift Distance Correction calibrated on data
  //
  //-------------------------------------------------------------------------------------

  // Parameters for new correction
  kCorr_EAngleBinSizeA = ( 2.0 * kCorr_EAngleMaxA) / ( (float) ( kCorr_EAngleBinsA - 1 ) ) ; 
  kCorr_TimeBinSizeA   = kCorr_TimeMaxA            / ( (float) ( kCorr_TimeBinsA   - 1 ) ) ;

   //-------------------------------------------------------------------------------------
      int setDriftA[kCorr_EAngleBinsA][kAmbiguitiesA][kCorr_TimeBinsA] ;

      for ( int loop_eangl=0 ; loop_eangl<kCorr_EAngleBinsA ; ++loop_eangl )
	{
	  for ( int loop_timeb=0 ; loop_timeb<kCorr_TimeBinsA ; ++loop_timeb )
	    {
	      int loop_ambig = 0 ;
	      corrDriftA[ loop_eangl ]
		        [ loop_ambig ]
		        [ loop_timeb ] = 0.0 ;
	       setDriftA[ loop_eangl ]
		        [ loop_ambig ]
		        [ loop_timeb ] = 0   ;
	      loop_ambig = 1 ;
	      corrDriftA[ loop_eangl ]
  ...
  */
  //  report(DEBUG, kFacilityString )  << "Garfield Drift Correction File Size: " 
   ...
   kActualFileSize = m_dataHolder->DriftCorr().size();

   report(INFO, kFacilityString )  << "Drift Correction File Size: " 
		 		   << kActualFileSize
			 	   << endl;

// Swap between DATA and GARFIELD correction maps using THE SIZE of file with constants

  if(kActualFileSize == kCorr_FileSizeA) 
  { 
    report(INFO, kFacilityString )  
    << "Load constants for new (calibrated from data) drift dist. vs time and E.A. correction" 
    << endl;
    report(INFO, kFacilityString ) << "Entrance Angle Correction Parameters "
	 	 		   << "(for calibrated on data map): "      << endl
		 		   << " Entrance Angle: Range(degree) = +-" << kCorr_EAngleMaxA
		 		   << ", # of bins ="                       << kCorr_EAngleBinsA
		 		   << ", Bin Size ="                        << kCorr_EAngleBinSizeA
		 		   << endl
		 		   << " Drift Time: Range(ps) = from 0 to " << kCorr_TimeMaxA
		 		   << ", # of bins ="                       << kCorr_TimeBinsA
		 		   << ", Bin Size ="                        << kCorr_TimeBinSizeA
		 		   << endl;
  ...
  for(FAConstants<DBDRGarfield_DistanceCorrection>::const_iterator 
        ...
        loopD!=m_dataHolder->DriftCorr().end()   ; ++loopD ) 
    {
        ...
        if ( angle < -kCorr_EAngleMaxA )         { error +=  1 ; }
        if ( angle >  kCorr_EAngleMaxA )         { error +=  2 ; }

	int anglebin  = ( (int) ( ( (angle + kCorr_EAngleMaxA)/ kCorr_EAngleBinSizeA ) + 0.5 ) ) ;
        int timebin   = ( (int) ( ( (*loopD).get_drifttime()  / kCorr_TimeBinSizeA   ) + 0.5 ) ) ;
        int ambiguity = (*loopD).get_z_position();

        // Range and Error Checking
        // Dont store data from out of range points!
        //
        if ( ambiguity != 0 && ambiguity != 1 )  { error +=  4 ; }
        if ( timebin  <             0  )         { error +=  8 ; }
        if ( timebin  > kCorr_TimeBinsA  )       { error += 16 ; }
	if ( setDriftA[anglebin ]
	              [ambiguity]
	              [timebin  ] != 0 )         { error += 32 ; }
	...
	if ( error == 0 )
	     ...
	     report( DEBUG, kFacilityString )  
                                            << " Rec:" << (*loopD).get_entry() 
                                            << " t:"   << (*loopD).get_drifttime()
                                            << " amb:" << (*loopD).get_z_position()
                                            << " ea:"  << (*loopD).get_entrance_angle()
                                            << " cor:" << (*loopD).get_driftdistance_correction()
                                            << " ia:"  << anglebin
                                            << " it:"  << timebin
                                            <<  endl;	
          }
  ...
  { 
    ...
    report(INFO, kFacilityString )  
    << "Load constants for old (GARFIELD calculated) drift dist. vs time correction" 
    << endl;
  ...
  */
  ...
  if(kActualFileSize == kCorr_FileSizeA)
   {                                     // For new (calibrated on data) E.A. correction 
         int Nbins_total;
         int Nbins_zero;
         float cor_Max;
         float cor_Min;
      ...
      for( int ambiguity=0 ; ambiguity < 2 ; ambiguity++ )
        {
         Nbins_total=0;
         Nbins_zero=0;
         cor_Max=0.;
         ...
         for( int angle_bin=0 ; angle_bin < kCorr_EAngleBinsA ; angle_bin++ )
           {
            for( int time_bin=0 ; time_bin < kCorr_TimeBinsA ; time_bin++ )
              {
                Nbins_total++;                 

	        if(setDriftA[angle_bin][ambiguity][time_bin] == 1 )
                 {
         	  correction = corrDriftA[angle_bin][ambiguity][time_bin];
                  if( correction == 0. )    { Nbins_zero++; } 
		  if( correction > cor_Max) { cor_Max = correction;}	     
		  if( correction < cor_Min) { cor_Min = correction;}	     
                 }
                else
                 {
                  report( ERROR, kFacilityString ) << " Problem with SET correction for node: "
                                                   << " ambiguity=" << ambiguity
                                                   << " angle_bin=" << angle_bin
                                                   << " time_bin=" << time_bin
                                << " # of resets=" << setDriftA[angle_bin][ambiguity][time_bin]
                                                   << endl;
                 }
              }
           }
         report( INFO, kFacilityString ) 
         << "E.A. Correction Statistics for Ambiguity = " << ambiguity << ":"
         << "\n                                     # of zero bins / Total=" << Nbins_zero
					                            << " / " << Nbins_total
         << "\n                                     Max/Min correction(m) =" << cor_Max
                                                                    << " / " << cor_Min 
         << endl;  
        }
   }
  else
   {  // For old (GARFIELD) E.A. correction
						        ...
						        float zHit ) const
{
  ...
  // This function makes a swap between old (GARFIELD) and new (DATA calibrated) maps

    static int NEntry=0;
               NEntry++;

  //if(NEntry<=100) 
    report( DEBUG, kFacilityString ) << " T="     << time
                                     << " L="    << layer
                                     << " Amb="  << ambiguity
                                     << " EA="   << eAngle
                                     << " zHit=" << zHit  << endl;

  // Swap between old and new version here...

  if( kActualFileSize == kCorr_FileSizeA )
    {
      if(NEntry==1) 
        {report( INFO, kFacilityString ) << "NEW (calibrated on data) version of time2DriftCorr" << endl;}

      return time2DriftCorr( time, ambiguity, eAngle );
    }
  else
    {
      if(NEntry==1) 
        {report( INFO, kFacilityString ) << "OLD (GARFIELD) version of time2DriftCorr" << endl;}

      return time2DriftCorr( time, layer, ambiguity, eAngle );
    }
}
						        ...
						        float eAngle ) const
{
  ...
  //  2003/09/25 D.M.: This version of time2DriftCorr works with constants calibrated from data
  ...
  if ( eAngle < -kCorr_EAngleMaxA ) 
  ...
  else
    {
      if ( eAngle > kCorr_EAngleMaxA )
	{ 
	  anglebin = kCorr_EAngleBinsA - 2 ; // counts from 0!
	  dangle   = 1.0                  ;
	}
      else
	{ // assumes truncation runs 0.9999>0 
          dangle   = ( ( eAngle + kCorr_EAngleMaxA ) / kCorr_EAngleBinSizeA ) ;
	  anglebin = ( (int) dangle )                                       ;
	  dangle   = ( dangle - ( (float) anglebin ) )                        ; 
	}
    }
  ...
  if ( time < 0 ) 
    ...
    {
      if ( time > kCorr_TimeMaxA )
	{ 
	  timebin = kCorr_TimeBinsA - 2 ; // counts from 0!
          dtime   = 1.0                ; 
	}
      else
	{ // assumes truncation runs 0.9999>0 
          dtime   = ( time / kCorr_TimeBinSizeA );
          timebin = ( (int) dtime )             ;
  ...
  // BiLinear Interpolation 
  //
  float result = (corrDriftA[anglebin  ][ambiguity][timebin  ]*( (1.0-dtime)  * (1.0-dangle) )) +
                 (corrDriftA[anglebin+1][ambiguity][timebin  ]*( (1.0-dtime)  *    dangle    )) +
                 (corrDriftA[anglebin  ][ambiguity][timebin+1]*(   dtime      * (1.0-dangle) )) +
                 (corrDriftA[anglebin+1][ambiguity][timebin+1]*(   dtime      *    dangle    )) ;


//    static int PrintCounter=0;
//    if(PrintCounter<100) { PrintCounter++; }

   if  ( debugLevel[6] )
    {
      report( INFO , kFacilityString )  << " result   " << result   
					<< " timebin  " << timebin  
					<< " anglebin " << anglebin
					<< " dtime    " << dtime   
					<< " dangle   " << dangle 
  ...
  // 
  //return -result;
  // M.D.: no need to swap the sign here... Sign is negative at call of this function   
...
DriftFunction::Drift DataDriftFunction::time2DriftCorr( Time  time, 
						        ...
						        float eAngle ) const
  //						        float zHit ) const
  //  2003/09/25 D.M.: The old (GARFIELD based) version of this function
{
  //
  // CHECK LAYER IN VALID RANGE
  // 
