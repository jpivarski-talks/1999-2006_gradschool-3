...
// $Id: ZDDataDriftFunction.cc,v 1.1.1.1 2003/06/26 19:02:49 huanggs Exp $
//
// Revision history
//
...
static const char* const kFacilityString   = "ZDDriftFunctionProd.ZDDataDriftFunction" ;
...
//----------------------------------------------------------------------------
ZDDataDriftFunction::ZDDataDriftFunction(ZDDriftFunctionProd* ZDDriftFunctionProd,
				     ZDDFDataHolder* iDataHolder): 
          ZDDriftFunction(ZDDriftFunctionProd, iDataHolder),
  ...
  if  ( debugLevel[4] )
      ...
      if ( threesided ) 
	{
	  for ( ilayer = 1; ilayer<=kmaxZDlayer; ++ilayer )
	    {
	      const ZDSinglelayer_df_data& ndf = zd_df_data[kLRaveraged-1][ilayer-1] ;
      ...
      if ( threesided )
	{
	  for ( ilayer = 1; ilayer<=kmaxZDlayer; ++ilayer )
	    {
	      const ZDSinglelayer_df_data& ndf = zd_df_data[kleftside-1][ilayer-1] ;   
	  ...
	  for ( ilayer = 1; ilayer<=kmaxZDlayer; ++ilayer )
	    {
	      const ZDSinglelayer_df_data& ndf = zd_df_data[krightside-1][ilayer-1] ;   
