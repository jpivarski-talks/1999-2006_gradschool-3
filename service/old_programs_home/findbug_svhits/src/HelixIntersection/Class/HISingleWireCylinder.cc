...
// $Id: HISingleWireCylinder.cc,v 1.12 2001/09/26 08:04:13 wsun Exp $
//
// Revision history
//
   ...
   const HIHelix& aHelix, HepVector& outputVector ) const
{
   ...
   if( aHelix.reportFitResultsAtInitRefPt() )
   {
      ...
      Hep3Vector bField = HIMagField::instance()->bfield( pos ) ;
	 ...
	 totalRadialArcLength * outputVector( HIHelix::kZ0 ) ;
      ...
      // d(DCA)/d(phi0) = V.dot( X.cross( bField.unit() ) )
   ...
   // d(DCA)/d(d0) = momentumAtInit.cross( bField ) ).unit().dot( V )
   //              = 1 for axial wires.
   // Unit vector perpendicular to both helix momentum and bfield.
