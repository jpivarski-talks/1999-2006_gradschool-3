...
// $Id: HIHelix.cc,v 1.16 2001/03/26 06:58:54 wsun Exp $
//
// Revision history
//
// $Log: HIHelix.cc,v $
...
HIHelix::HIHelix( const HIHelix& rhs )
   : KTHelix( rhs ),
     m_mass(              rhs.m_mass ),
...
const HIHelix& HIHelix::operator=( const HIHelix& rhs )
{
   if( this != &rhs )
   {
      KTHelix::operator=( rhs ) ;
      m_mass              = rhs.m_mass ;
      m_lastArcLength     = rhs.m_lastArcLength ;
      m_totalArcLength    = rhs.m_totalArcLength ;
      m_identifier        = rhs.m_identifier ;
