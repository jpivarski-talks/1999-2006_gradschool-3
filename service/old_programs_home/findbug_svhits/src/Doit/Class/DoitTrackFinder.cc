   ...
   const Frame& theFrame ) const
{
   ...
   // Let's extract the FATable of CollatedRawDRHit
   FATable<CollatedRawDRHit> rawHits;
   extract( theFrame.record( Stream::kEvent ), rawHits );

   // Did we get any
   if ( !rawHits.valid() )
   {
     // Just print a message
     report(INFO, kFacilityString) << "No raw hits in event. "
				   << "TDCs will be zero." << endl;
   }
   ...
   if ( theDRHits != 0 )
   {
	 ...
	 // Get the CollatedRawDRHit for this CalibratedDRHit


	 const CollatedRawDRHit& rawHit = rawHits[aDRHit->identifier()];

	 // Get the TDC
	 if ( rawHit.digiTimes().size() > 0 ) 
	 {
	   int which= aDRHit->whichTDC();

	   if ( which >= 0 && which < rawHit.digiTimes().size() )
	   {
	     tdc = rawHit.digiTimes()[which];
	   }
	   else
	   {
	     report(EMERGENCY, kFacilityString) 
	       << "Huh? Which TDC hit out of range? \n"
	       << "which = " << which << "  size = " 
	       << rawHit.digiTimes().size() << endl;
	     tdc = 0;
	   }
