...
// $Id: DoitStatusInfo.h,v 1.2 2001/01/05 06:30:31 lyon Exp $
//
// Revision history
//
...
class DoitStatusInfo
{
   ...
   public:
	...
	return isTrue(1, 30); } 
      ...
      //! Did we test TFXFIT-LF with z0 constraint (word2, bit29)
      DABoolean testedTFXFITLFWithZ0Constraint() const { 
	return isTrue(2, 29); };

      //! Did we use TFXFIT-LF with z0 constraint (word2, bit30)
      DABoolean usedTFXFITLFWithZ0Constraint() const { return isTrue(2, 30); };

      //! Did we boot silicon z hits for curlers (word2, bit31)
      DABoolean bootSiliconZ() const { return isTrue(2, 31); };
