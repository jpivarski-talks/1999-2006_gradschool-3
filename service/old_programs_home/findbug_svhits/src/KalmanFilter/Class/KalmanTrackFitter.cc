...
// $Id: KalmanTrackFitter.cc,v 1.51 2001/10/09 17:39:26 llh14 Exp $
//
// Revision history
//
			     ...
			     DBCandidate::Hypo aHypo )
{
   ...
   if( m_outwardFit != 0 )
   {
	 ...
	 m_outwardFit ) ;
      ...
      // NavTrack matches seeds and fits by identifier.
      m_outwardFit->setIdentifier( theSeed->identifier() ) ;

      // fittedTrack() should not delete the outward fit.
      m_deleteOutwardFit = false ;
   }
...
KalmanTrackFitter::fittedTrack( const TRSeedTrack* theSeed,
				...
				DBCandidate::Hypo aHypo )
{
   KTHelix* inwardFit = fittedTrack( theSeed,
				     theFrame,
				     DBCandidate::mass( aHypo ) ) ;
   ...
   {
      delete m_outwardFit ;
      m_outwardFit = 0 ;  // prevents fittedTrack() from deleting again.
      return inwardFit ;
   }
   else
   {
...
//   intersector.setAttemptCurling( false ) ;
   ...
   // Fit outward then inward.  Based on smoothed residuals, discard hit
   ...
   for( int iHitDel = 0 ; iHitDel <= imax ; ++iHitDel )
   {
	 ...
	 hitItr->hiCalibratedHit()->clear() ;
      ...
      if( m_outwardFit != 0 )
      {
	 hitItr = m_outwardFit->hitsOnTrack().begin() ;
	 hitEnd = m_outwardFit->hitsOnTrack().end() ;
	 ...
	 m_outwardFit->reset() ;
      ...
      // m_outwardFit (which is actually the inward fit if m_inputTrackInfo
      // is kAddHitsToInputTrackInside).  So, later on, m_outwardFit and
	 ...
	 if( m_outwardFit == 0 )
	 {
	    m_outwardFit = new KalmanHelix( *aHelix ) ;
	 }
	 else
	 {
	    *m_outwardFit = *aHelix ;
	 ...
	 m_outwardFit->updateTrackParameters() ;
	 m_outwardFit->setFitFailed( false ) ;
	 ...
	 if( m_outwardFit == 0 )
	 {
	    m_outwardFit = new KalmanHelix( copyOfOriginal ) ;
	 }
	 else
	 {
	    *m_outwardFit = copyOfOriginal ;
	 }

	 m_outwardFit->setFitFailed( true ) ;
	 ...
	 *aHelix = copyOfOriginal ;
	 aHelix->moveToRadius( intersector.currentSurface()->radius() ) ;
      ...
      // Don't clear the HICalibratedHits if outward fit fails because
	 ...
	 *aHelix = copyOfOriginal ;
	 aHelix->setFitFailed( true ) ;
         ...
         //Store the link data of the outward fit if that succeeded
	 STL_VECTOR( HIFitHelix::HitAndLinkData )::iterator hitItr =
	    m_outwardFit->hitsOnTrack().begin() ;
	 STL_VECTOR( HIFitHelix::HitAndLinkData )::iterator hitEnd =
	    m_outwardFit->hitsOnTrack().end() ;         
	 ...
	 return aHelix ;
      ...
      // kAddHitsToInputTrackInside, the inward fit is actually m_outwardFit.

      if( !m_inwardFitsAtLastHit )
      {
	 if( m_inputTrackInfo == kAddHitsToInputTrackInside )
	 {
	    m_outwardFit->moveToReferencePoint(
	       HepPoint3D( 0., 0., 0. ),
	       KTMoveControl::kDirectionBackward ) ;
	 }
	 else
	...
	if ( theSeed != 0 ) 
	{
	 ...
	 if( m_outwardFit != 0 )
	 {
	    cout << " chisq/ndof out " << m_outwardFit->chisq()
		 << "/" << m_outwardFit->ndof() ;
	 }
	 if( aHelix != 0 )
	 {
...
//       if( iHitDel < imax )
   ...
   m_outwardFit->setNumberHitsDropped(  m_nHitsDropped ) ;
   m_outwardFit->setNumberHitsFlipped(  m_nHitsFlipped ) ;
   m_outwardFit->setFitQualityInfo(     m_moreBadHits, m_pullOfNextWorstHit ) ;
   ...
   if( m_fitConstraint != 0 )
   {
	 ...
	 m_compositeConstraint->applyConstraint( m_outwardFit ) ;
      }
      else
      {
	 m_fitConstraint->applyConstraint( aHelix ) ;
	 m_fitConstraint->applyConstraint( m_outwardFit ) ;
      }
   }
   else if( parametersFixed )
   {
      m_fixParamsConstraint->applyConstraint( aHelix ) ;
      m_fixParamsConstraint->applyConstraint( m_outwardFit ) ;
   }
   ...
   // Swap m_outwardFit and aHelix if hits were added to a prefit track.
   if( m_inputTrackInfo == kAddHitsToInputTrackInside ||
       m_inputTrackInfo == kAddHitsToInputTrackOutside )
   {
      delete aHelix ;
