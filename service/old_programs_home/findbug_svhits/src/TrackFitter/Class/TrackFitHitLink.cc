...
// $Id: TrackFitHitLink.cc,v 1.4 2000/08/16 19:34:22 wsun Exp $
//
// Revision history
//
...
static const char* const kIdString  = "$Id: TrackFitHitLink.cc,v 1.4 2000/08/16 19:34:22 wsun Exp $";
...
TrackFitHitLink::TrackFitHitLink( const TrackFitHitLink& rhs )
   : m_used(          rhs.m_used ),
     m_residual(      rhs.m_residual ),
     m_residualError( rhs.m_residualError ),
//     m_curvature(     rhs.m_curvature ),
     m_momentum(      rhs.m_momentum )
{
}

const TrackFitHitLink& TrackFitHitLink::operator=( const TrackFitHitLink& rhs )
{
   if( this != &rhs )
   {
      m_used = rhs.m_used ;
