...
CompareInOutChi2::hist_book( HIHistoManager& iHistoManager )
{
   ...
   const char* label[kNumEntries] = {
      "ntracks", 
      "pused", "pin", "pdrop", "piter", "pfit",
      "eused", "ein", "edrop", "eiter", "efit"};
...
CompareInOutChi2::event( Frame& iFrame )          // anal3 equiv.
{
         ...
         ++trackItr )
   {
      ...
      v[kNTracks] = ntracks;
      v[kPionUsed] = trackItr->pionQuality()->numberSubdetectorHits(TRSubdetectorLists::kEntireDetector);
      v[kPionIn] = trackItr->pionQuality()->numberHitsInput();
      v[kPionDrop] = trackItr->pionQuality()->numberHitsDropped();
      v[kPionIter] = trackItr->pionQuality()->numberIterations();
      v[kPionFit] = (trackItr->pionQuality()->fit()  &&  !trackItr->pionQuality()->fitAbort());
      v[kExitUsed] = trackItr->exitPionQuality()->numberSubdetectorHits(TRSubdetectorLists::kEntireDetector);
      v[kExitIn] = trackItr->exitPionQuality()->numberHitsInput();
      v[kExitDrop] = trackItr->exitPionQuality()->numberHitsDropped();
      v[kExitIter] = trackItr->exitPionQuality()->numberIterations();
      v[kExitFit] = (trackItr->exitPionQuality()->fit()  &&  !trackItr->exitPionQuality()->fitAbort());
