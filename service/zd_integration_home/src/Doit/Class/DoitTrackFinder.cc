...
// Revision 1.71  2003/06/28 23:58:50  cdj
// changed call to fillFortranCDGeometry to match new argument list
//
// Revision 1.70  2003/06/26 18:24:29  huanggs
// add ZD codes
//
// Revision 1.69  2003/04/11 20:36:46  bkh
...
void DoitTrackFinder::beginRun(Frame& theFrame) const
{
   ...
   extract( theFrame.record(Stream::kZDAlignment), zdStore);
   ...
   // Fill cdgeomcd common block
	 ...
	 << "DoitTrackFinder::beginrun, before fillFortranCDGeometry" 
	 << "m_useSilicon="
	 << m_useSilicon <<" "
	 << "iConfig->useSInotZD()="
	 << iConfig->useSInotZD() <<" "
   ...
   fillFortranCDGeometry(*senseWireStore, *cathodeStore, &(*zdStore),
                         m_useSilicon && iConfig->useSInotZD(),
                         m_useZD && iConfig->useZDnotSI() );
   ...
   ) const
{
   ...
   FAItem< DetectorConfiguration >                              iConfig   ;
   extract( theRecord.frame().record( Stream::kBaseGeometry ) , iConfig ) ;
   ...
   const Frame& theFrame ) const
{
   ...
   if ( theDRHits != 0 )
   {
     for ( unsigned int hitcounter = 0;
	   hitcounter < theDRHits->size(); ++hitcounter )
     {

       const CalibratedDRHit* aDRHit = (*theDRHits)[hitcounter];
	 ...
	 phi -= 2.0*M_PI;
       ...
       int idx;
       int device = kDoitDeviceDR;
       // Fill this hit in the common blocks
       doitloaddrhits_(&device,
		       &layer, &wire, &tdc, &time, &charge, &phi, &distance,
	...
	report(INFO,kFacilityString) 
       ...
       // Check that we know what we are doing; idx should be the number
       // of locations filled in the Fortran - a global value including
       // any subdetector (DR,ZD...) that is added to the array. 
       // A test of idx, here, would be redundant; idx is incremented
       // when there is room in the Fortran array. However, if there is
       // no room, doitloaddrhits had a STOP.

       // Do we have to add to the vector?
       if ( m_hitIdentifiers.size() <= hitcounter )
       {
	  const_cast<DoitTrackFinder*>(this)->m_hitIdentifiers.
       ...
       {
	  const_cast<DoitTrackFinder*>(this)->m_hitIdentifiers[hitcounter]
	     = hitId;
       }

     } // loop over DR hits; hitcounter
   ...
   const Frame& theFrame ) const
{
   ...
   // Are there any ZD hits?
   if ( theZDHits != 0 )
   {
     for ( unsigned int hitcounter = 0;
	   hitcounter < theZDHits->size(); ++hitcounter )
     {

       const CalibratedDRHit* aZDHit = (*theZDHits)[hitcounter];
	 ...
	 phi -= 2.0*M_PI;
       ...
       int idx;
       int device = kDoitDeviceZD;

       // Fill this hit in the common blocks
       // use the same load routine for ZD and DR
       doitloaddrhits_(&device,
		       &layer, &wire, &tdc, &time, &charge, &phi, &distance,
		       &xpos, &ypos, &cdla, &qual, &earlyGap,
		       &lateGap, &ntdc, &idx);
	...
	report(INFO,kFacilityString) 
       ...
       // Do we have to add to the vector?
       if ( m_ZDhitIdentifiers.size() <= hitcounter )
       {
	  const_cast<DoitTrackFinder*>(this)->m_ZDhitIdentifiers.
       ...
       {
	  const_cast<DoitTrackFinder*>(this)->m_ZDhitIdentifiers[hitcounter]
	     = hitId;
       }
     } // loop over ZD hits; hitcounter
...
// Extract Si hits
   ...
   for ( int i = 1; i <= nTracks; ++ i )
   {
	...
	if( device == kDoitDeviceDR ) 
	{
	  ...
	  if ( diagLevel > 0 )
	     report ( INFO, kFacilityString ) << "DR3Hit on track " << i 
					      << " " 
					      << "iadr= " << iadr << " " 
					      << "Layer= " << layer << " " 
					      << "wire= " << wire << " "
	...
	else if ( device == kDoitDeviceZD )
	{
	  ...
	  if ( m_ZDhitIdentifiers.size() <= (iadr-1) )
	  {
